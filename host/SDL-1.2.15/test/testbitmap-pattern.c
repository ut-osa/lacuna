...
int main(int argc, char *argv[])
{
	...
	if (argc<=1 ){ printf("No token provided  \n"); return -1;}

        if (argc<TOKENS) {            printf("token should be at least %d symbols, space-dlimited\n",TOKENS); return -1;}
        for(i=0;i<TOKENS;i++){
                token[i*2]=atoi(argv[1+i]);
                token[i*2+1]=0;
                printf("%c\n",token[i*2]);
        }
...
*/		}
	...
	if (screen->format->BytesPerPixel!=2) {
		int cnt=0;
        	for ( i=0; i<(buffer_size);i++,cnt=(cnt+2)%(TOKENS*2)) {
	        	buffer[i]=token[cnt];
        	}
        }
