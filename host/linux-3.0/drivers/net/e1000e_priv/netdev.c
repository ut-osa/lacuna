...
{
    //yxu: to vm
    void* idptr = privnet_find_mac(skb->data);
    u16 tag;
    if (idptr)
    {
        unsigned char *ch = skb->data + ETH_HLEN - 1;
        //the last byte in ETH_H
        *ch += 1;
        ch++;
        privnet_encrypt(ch, skb_tail_pointer(skb) -  ch, idptr);
    }

	tag = le16_to_cpu(vlan);
			       ...
			       int *work_done, int work_to_do)
{
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		/* AD: UN-UH, not in MY house (this is a privacy leak...) */
#if 0
		/*
...
/**
 * yxu: zero out the frame if it comes from VM
*/
static inline void erase_vm_frame(struct sk_buff *skb)
{
    if (!skb)
        return;
    //struct ethhdr *ehd = (struct ethhdr *) skb->data;
    if (unlikely(privnet_find_mac(skb->data + 6)))
			     ...
			     struct e1000_buffer *buffer_info)
{
	...
	if (buffer_info->skb) {
        //yxu
        erase_vm_frame(buffer_info->skb);
			   ...
			   int tx_flags, int count)
{
	...
	do {
        ...
        //yxu: from vm (decrypt now)
        if (buffer_info->skb && unlikely(privnet_find_mac(
                        buffer_info->skb->data + 6)))
        { 
            void* idptr = privnet_find_mac(
                        buffer_info->skb->data + 6);
            if (idptr) {
                unsigned char *ch = buffer_info->skb->data + ETH_HLEN - 1;
                //the last byte in ETH_H
                *ch -= 1;
                ch++;
                privnet_decrypt(ch, 
                        skb_tail_pointer(buffer_info->skb) - ch, idptr);
            }
        }
