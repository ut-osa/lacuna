...
static int usb_qdev_exit(DeviceState *qdev)
{
    ...
    if (dev->attached) {
        /* AD: It appears that without usb_device_detach commented
           out, USB host devices don't get reset properly (they need
           an extra reset, which I fear was source of other errors
           like completions for URBs that should've been cancelled
           already).  I'm not sure how this affects emulated USB
           devices. */
...
int usb_device_attach(USBDevice *dev)
{
    ...
    /* AD: This seems to be the issue when there are too few ports
       free on the simulated root USB hub - connections to the new
       simulated hub don't seem to show up in the guest */
