...
static int hotplug_needs_reset = 1;
void set_hotplug_needs_reset(int status)
{
    hotplug_needs_reset = status;
}

int get_hotplug_needs_reset(void)
{
    return hotplug_needs_reset;
}
				...
				PCIHotplugState state)
{
    ...
    if (get_hotplug_needs_reset()) {
#ifdef CONFIG_PRIV_INPUT
        /* To keep QEMU at the default behavior without PRIV_INPUT,
           pci0_status.{up,down} being reset should happen every time
           we reach this function.  However, this screws up
           simultaneous multiple device assignment (which Lacuna uses)
           because the assignment of the second device clears out the
           results from the first. */
        set_hotplug_needs_reset(0);
#endif
        s->pci0_status.up = 0;
