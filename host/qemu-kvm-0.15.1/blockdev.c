...
DriveInfo *drive_init(QemuOpts *opts, int default_to_scsi)
{
    ...
    if (bdrv_key_required(dinfo->bdrv)) {
        ...
        if (priv_diff) {
            char random_key[17];
            int i;
            /* This is another case where we are (knowingly) using a
               bogus random number generation method */
            fprintf(stderr, "Using bogus RNG for private diffs\n");
            srandom(time(NULL));
            for (i = 0; i < 16; i++) {
                random_key[i] = (char)(random()%95) + ' ';
            }
            random_key[16] = '\0';
            
            bdrv_set_key(dinfo->bdrv, random_key);
        }
        else {
            autostart = 0;
        }
