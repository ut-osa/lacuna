...
// WORKS only for the 128bit keys
...
struct bigint plus(struct bigint x, uint y){        
	...
	struct bigint res;
        ...
        for(i=3;i>=0;i++){
                if (MAXVAL != x.d[i] && MAXVAL- carry - x.d[i] >= y) {         
                        res.d[i]=x.d[i]+y+carry;
                        carry=0;                                               
                        break;
                }                                                              
                res.d[i]=y-(MAXVAL+1-x.d[i]-carry);
                y=0;
                carry=1;                                                       
        }
...
{
   return gcry_cipher_setkey(hd, key, AES_KEY_SIZE);
}

int cpu_encrypt_string_count(uchar* in, int in_len,  struct bigint ctr )
{
 int r=0;	
 if ((r=gcry_cipher_setctr(hd, ((char*)(ctr.d)), sizeof(ctr))) != 0){
      errx(1, "gcry_cipher_setctr");
	return r;
 }

 if ((r=gcry_cipher_encrypt(hd, (unsigned char *)in, in_len, NULL, 0)) != 0){
      errx(1, "gcry_cipher_encrypt");
	return r;
 }
}

int cpu_decrypt_string_count(uchar* in, int in_len,  struct bigint ctr ){
 int r;
 if ((r=gcry_cipher_setctr(hd, ((char*)(ctr.d)), sizeof(ctr))) != 0){
      errx(r, "gcry_cipher_setctr");
	return r;
 }

 if ((r=gcry_cipher_decrypt(hd, (unsigned char *)in, in_len, NULL, 0)) != 0){
      errx(r, "gcry_cipher_encrypt");
	return r;
 }
}
void cpu_init(){
	...
	char iv[AES_KEY_SIZE]={0};
   if (gcry_cipher_setiv(hd, iv, AES_KEY_SIZE) != 0)
      errx(1, "gcry_cipher_setiv");
}
