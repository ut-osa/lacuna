...
extern "C" int aesni_set_key(struct aes_ctx *ctx, const uint8_t *in_key,
                         unsigned int key_len);

extern "C" void aesni_ctr_enc(struct aes_ctx *ctx, const uint8_t *dst, uint8_t *src,
...
struct bigint plus(const struct bigint x, uint y){
        char i=0;                                                              
        char carry=0;                                                          
	struct bigint res;
        res.d[0]=x.d[0];
...
{
   assert(hd);
   return aesni_set_key(hd, key, AES_KEY_SIZE);
}
...
int cpu_encrypt_string_count(uchar* in, int in_len,  struct bigint ctr )
{
 ...
 char* noencrypt=getenv(NO_ENCRYPT_ENV);
 if (!noencrypt){
	 aesni_ctr_enc(hd, in, in, (size_t)in_len, (uint8_t*)ctr.d);
 }
 return 0;
}
...
int cpu_decrypt_string_count(uchar* in, int in_len,  struct bigint ctr ){
 ...
 char* noencrypt=getenv(NO_ENCRYPT_ENV);
 if (!noencrypt){
 	return cpu_encrypt_string_count(in, in_len,ctr);
 }
 return 0;
}
void cpu_init(){
 assert(!posix_memalign((void**) &hd,16,sizeof(struct aes_ctx)));
}
